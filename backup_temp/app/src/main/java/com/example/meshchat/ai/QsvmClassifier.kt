...
class QsvmClassifier(context: Context? = null) {
    ...
    private val gpuOptions by lazy { 
        val options = Interpreter.Options()
        if (compatList.isDelegateSupportedOnThisDevice) {
            val delegateOptions = compatList.bestOptionsForThisDevice
            options.addDelegate(GpuDelegate(delegateOptions))
            Log.i("QSVM", "Hardware Acceleration: GPU Delegate Enabled")
        } else {
            options.setNumThreads(4)
            Log.w("QSVM", "Hardware Acceleration: GPU Not Supported, falling back to 4-Thread CPU")
        }
        options
    }
    ...
    private val classBiases = mapOf(
        1 to 0.45f, // Higher baseline preference for Fast mode to prevent false positives
        2 to 0.20f,
        3 to 0.15f,
        4 to 0.10f
    )
    ...
    fun extractFeatures(message: String, context: List<String> = emptyList()): DoubleArray {
        ...
        // Extreme Adversarial Normalizer: strip invisible/control chars, spaces, punctuation
        ...
        val pqMatches = pqKeywords.count { normalizedLeetspeak.contains(it) }
        ...
        val authMatches = authKeywords.count { normalizedLeetspeak.contains(it) }
        ...
        // Match standard floats, N/S E/W coordinates, or DMS
        val hasGps = Regex("[-+]?[0-9]{1,3}\\.[0-9]{1,7}").containsMatchIn(message) || Regex("[0-9]{1,3}°[0-9]{1,2}'[0-9]{1,2}(?:\\.[0-9]+)?\"[NS]\\s*[0-9]{1,3}°[0-9]{1,2}'[0-9]{1,2}(?:\\.[0-9]+)?\"[EW]").containsMatchIn(message)
        val hasGpsKey = Regex("lat|lon|gps|waypoint|rendezvous|coords|coordinates").containsMatchIn(normalizedLeetspeak)
        var gpsScore = 0.0
        ...
        val piiMatches = piiKeywords.count { normalizedLeetspeak.contains(it) }
        var piiScore = if (piiMatches > 0) 0.65 + (piiMatches * 0.10) else 0.0
        if (hasEmail) piiScore += 0.45
        if (hasPhone) piiScore += 0.40
        if (normalizedLeetspeak.contains("keep this between") || normalizedLeetspeak.contains("secure")) piiScore += 0.30
    ...
    fun classify(message: String, context: List<String> = emptyList()): QsvmResult {
        ...
        val predictedTier = if (!hasSensitiveSignals) {
            1 // Default to Fast / Efficiency Mode for benign messages like "hi", casual conversation
        } else {
            // When sensitive signals are present, ensure we don't fall back to level 1, unless class 1 score is astronomically higher
            val topScoreEntry = boostedScores.maxByOrNull { it.value }
            val maxNonClass1 = boostedScores.filterKeys { it >= 2 }.values.maxOrNull() ?: 0f
            if (topScoreEntry != null && topScoreEntry.key == 1 && topScoreEntry.value > maxNonClass1 + 0.2f) {
                 // Even with a slight sensitive signal, if the vector firmly points to Class 1, let it be (could be false positive)
                 1
            } else {
    ...
    private fun extractSensitiveFactors(message: String): List<String> {
        ...
        val normalizedLeetspeak = spaceStripped
            ...
            .replace("$", "s")
            .replace("!", "i")
        val keywords = listOf("password", "passwd", "secret", "token", "location", "gps", "quantum", "key", "rendezvous", "auth", "confidential", "pin", "kyber")
        for (kw in keywords) {
            if (normalizedLeetspeak.contains(kw)) found.add("keyword:$kw")
        }
        if (Regex("[-+]?[0-9]{1,3}\\.[0-9]{1,7}").containsMatchIn(message) || Regex("[0-9]{1,3}°[0-9]{1,2}'[0-9]{1,2}(?:\\.[0-9]+)?\"[NS]\\s*[0-9]{1,3}°[0-9]{1,2}'[0-9]{1,2}(?:\\.[0-9]+)?\"[EW]").containsMatchIn(message)) {
