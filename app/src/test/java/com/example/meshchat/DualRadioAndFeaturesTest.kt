...
class DualRadioAndFeaturesTest {
    ...
    fun testHybridRadioTransportResolution() {
        val meshService = LoRaMeshService(context = null)

        // Offline hardware prioritization: LoRa active by default when Wi-Fi not connected
        val (activeTransport, detail) = meshService.resolveActiveTransport(
            requested = RadioTransport.HYBRID_AUTO,
            peerNearby = true
        )
        assertEquals(RadioTransport.LORA_MESH, activeTransport)
        assertTrue(detail.contains("LoRa Mesh"))
    }
    ...
    fun testIoTDeviceDiscoveryModel() {
        ...
        val loraRepeater = IoTDevice(
            ...
            radioType = RadioTransport.LORA_MESH
        )
